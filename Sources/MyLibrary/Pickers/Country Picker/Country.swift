...
struct Country: Equatable, Hashable, Identifiable {
    ...
    var countryName: String {
        ...
        return NSLocale(localeIdentifier: currentLocale).displayName(forKey: NSLocale.Key.identifier, value: id) ?? isoCode
    }
    ...
    static var allCountries: [Country] {
        ...
        return countries.sorted { (c1: Country, c2: Country) -> Bool in
            return c1.countryName < c2.countryName
        }
    }
    
    static var countryListSectionHeader: [Alphabet.Letter] {
        let alphabet = Alphabet.letters
        let countries = Country.allCountries
        var indexArray: [Alphabet.Letter] = []
        
        for letter in alphabet {
            
            for country in countries {
                
                if country.countryName.prefix(1) == letter.value {
                    
                    if !indexArray.contains(letter) {
                        indexArray.append(letter)
                    }
                }
            }
        }
